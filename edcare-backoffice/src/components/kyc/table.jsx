...
export default function Table({kycList}) {
    const columnNames = ['客戶編號', '帳號名稱', '身份', '真實姓名', '送審時間', '審核者/時間','狀態','詳細'];
            ...
            <div className="table-body">
                {kycList.map((kyc, index) => (
                    <div key={index} style={{width: '100%', display: 'flex'}}>
                        ...
                        <div className="table-body-column">
                            {kyc.created_ts ? new Date(kyc.created_ts).toLocaleDateString('zh-TW', { timeZone: 'Asia/Taipei' }) : '-'}
                        </div>
                        <div className="table-body-column">
                            {kyc.update_ts ? new Date(kyc.update_ts).toLocaleDateString('zh-TW', { timeZone: 'Asia/Taipei' }) : '-'}
                        </div>
                        <div className="table-body-column">
                            {kyc.status === 'approve' ? (
