...
export default function Table({ adminList, columnNames }) {
  ...
  const updateAdmin = async () => {
    console.log("即將送出 updateAdmin，site:", site);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        !event.target.closest(".admin-create-modal")
      ) {
        setOpenModal(false);
        ...
        {adminList.map((admin, index) => (
          <div key={index} style={{ width: "100%", display: "flex" }}>
            ...
            <div className="table-body-column">
              <Switch
                isOn={admin.is_enable}
                onChange={() => handleModal(admin.id, !admin.is_enable)}
              />
            </div>
            <div className="table-body-column">
              {new Date(admin.created_ts).toLocaleDateString("zh-TW", {
          ...
          <div className="admin-create-modal">
            ...
            <div className="admin-create-modal-content">
              ...
              <div className="admin-create-button-wrapper">
                <button className="admin-create-button" onClick={updateAdmin}>
                  確認
                </button>
              </div>
            </div>
